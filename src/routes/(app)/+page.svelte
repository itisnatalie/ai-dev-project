<script>
	...
	$: ({ domains, size } = data);
	$: totalPages = Math.ceil(size / length);
	$: currentPage = Number($page.url.searchParams.get('skip') / length);
</script>
...
<section>
	...
	<ol class="w-full pt-0">
		{#each domains.docs as domain, index}
			<li class="group flex w-full items-center border-y bg-white py-1 hover:border-slate-400">
				...
				<div class="ml-auto flex items-center gap-12">
					<p
						...
						class:bg-yellow-200={domain.data().status == 'added'}
						class:bg-green-200={domain.data().status == 'finished'}
						class:bg-red-200={domain.data().status == 'aborted'}
						class:bg-blue-200={domain.data().status == 'scanning'}
						class:bg-yellow-300={domain.data().status == 'evaluating'}
					>
						{domain.data().status}
					</p>
					<a
	...
	{#each Array(totalPages) as _, index}
		<a href="/?skip={length * index}" class:text-sky-500={currentPage == index}>{index + 1}</a>
	{/each}
</section>
